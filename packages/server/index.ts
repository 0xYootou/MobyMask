...
const methodMapping = {
  submitInvocations: async (
    signedInvocations: SignedInvocation[]
  ): Promise<boolean> => {
    try {
...
async function deployContract() {
  const Registry = new ethers.ContractFactory(
    abi,
    phisherRegistryArtifacts.bytecode,
    signer
  );
  ...
  fs.writeFileSync(
    configPath,
    JSON.stringify(
      { address, name: _name, chainId: registry.deployTransaction.chainId },
      null,
      2
    )
  );
  try {
    ...
    console.log(
      "Deployment failed, trying to attach to existing contract.",
      err
    );
    throw err;
  }
...
async function signDelegation() {
  ...
  const delegation = {
    delegate: "0xF95555A29E58188147D3A3AcD6e2Ffeb04EA7dd5",
    authority:
